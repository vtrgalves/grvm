CREATE OR REPLACE FUNCTION public.create_or_sync_profile()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  _email text := NULLIF(_claims ->> 'email', '');
  _name text;
  _avatar text;
...
BEGIN
  ...
  _name := COALESCE(
    ...
    NULLIF(_meta ->> 'name', ''),
    NULLIF(_meta ->> 'preferred_username', ''),
  ...
  VALUES (
    ...
    _email,
    _avatar,
    'fan'::public.profile_type,
    2000,
    public.compute_level(2000)
  )
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    grv_points = CASE
      WHEN public.profiles.grv_points IS NULL OR public.profiles.grv_points < 0 THEN 2000
      ELSE public.profiles.grv_points
    END,
    level = public.compute_level(CASE
      WHEN public.profiles.grv_points IS NULL OR public.profiles.grv_points < 0 THEN 2000
      ELSE public.profiles.grv_points
    END),
    updated_at = now()
  ...
  WHERE NOT EXISTS (
    ...
    WHERE um.user_id = _uid AND um.mission_key = m.mission_key
  );
  ...
  RETURN jsonb_build_object(
    ...
    'avatar_url', _profile.photo_url,
    'photo_url', _profile.photo_url,
    'profile_type', _profile.profile_type,
    'grv_balance', _profile.grv_points,
    'grv_points', _profile.grv_points,
    'level', _profile.level,
    'xp', GREATEST(_profile.grv_points - CASE
      WHEN _profile.grv_points >= 50000 THEN 50000
      WHEN _profile.grv_points >= 25000 THEN 25000
      WHEN _profile.grv_points >= 10000 THEN 10000
      WHEN _profile.grv_points >= 4000 THEN 4000
      WHEN _profile.grv_points >= 1500 THEN 1500
      WHEN _profile.grv_points >= 500 THEN 500
      ELSE 0
    END, 0),
    'city', _profile.city,
    'selected_genres', _profile.selected_genres,
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'is_demo_fallback', jsonb_build_object(
      ...
      'nfts', jsonb_array_length(_nfts) = 0,
