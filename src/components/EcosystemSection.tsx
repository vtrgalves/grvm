...
const cards = [
  {
    ...
    glow: "hover:box-glow-blue hover:border-primary",
  },
  {
    icon: Radio,
    title: "Streaming Descentralizado",
    desc: "Liberdade para artistas e fãs",
    glow: "hover:box-glow-purple hover:border-secondary",
  },
  {
    icon: ShoppingBag,
    title: "Marketplace",
    desc: "Compre, venda e monetize experiências musicais",
    glow: "hover:box-glow-magenta hover:border-accent",
  },
  {
    icon: Gift,
    title: "Rewards",
...
const EcosystemSection = () => {
  ...
  return (
    <section ref={ref as React.RefObject<HTMLElement>} className="section-reveal py-24 md:py-32 relative">
      <div className="absolute top-1/3 right-1/4 w-[500px] h-[500px] rounded-full bg-secondary/5 blur-[150px]" />
      <div className="container mx-auto px-4 relative z-10">
        <h2 className="font-display text-2xl md:text-4xl lg:text-5xl font-bold text-center mb-16">
          Um ecossistema <span className="gradient-neon-text">completo</span>
        </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
          {cards.map(({ icon: Icon, title, desc, glow }, i) => (
            <div
              key={i}
              className={`flex flex-col items-center text-center p-8 rounded-2xl border border-border bg-card/50 backdrop-blur-sm transition-all duration-300 hover:scale-105 group ${glow}`}
            >
              <div className="p-4 rounded-full bg-muted mb-6 group-hover:scale-110 transition-transform">
